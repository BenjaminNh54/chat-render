...
let ws;
let reconnectTimeout;
let wasDisconnected = false; // flag pour reload après reconnexion
...
function connectWS() {
  ...
  try {
    ...
    ws.onopen = () => {
      ...
      // Si on avait été déconnecté avant → reload une seule fois
      if (wasDisconnected) {
    ...
    ws.onclose = () => {
      addMessage({ pseudo: 'Système', text: '⚠ Déconnecté. Reconnexion...' });
      wasDisconnected = true; // marquer la déconnexion
      reconnectTimeout = setTimeout(connectWS, 2000);
    };
