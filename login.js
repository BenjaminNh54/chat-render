...
//Il faut rafrechire la page discretement toutes les 3sec on va faire ça avec copilot mais je ne veux pas que visuelment le client voit que ça ce recharge
...
function selectGroup(group) {
    ...
    document.getElementById('connectGroupBtn').onclick = () => {
        // Logique de connexion au groupe (ex : redirection)
        alert('Connecté au groupe ' + group.name);
    };
    document.getElementById('requestAdminBtn').onclick = async () => {
        const userId = localStorage.getItem('userId');
        const response = await fetch(`${SERVER_URL}/group/${group.id}/request`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
